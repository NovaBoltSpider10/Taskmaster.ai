...
const Dashboard = () => {
      ...
      try {
        ...
        if (classesRes.data.length > 0) {
          ...
          const resourcePromises = classIds.map((id) =>
            axios
              .get<ResourceData[]>(
                `http://localhost:3000/resources/class/${id}`,
                { headers: { "x-auth-token": token } }
              )
          ...
          const taskPromises = classesRes.data.map(
            (
              ...
              axios
                ...
                .then((res) =>
                  res.data.map((t) => ({ ...t, className: c.name }))
                ) // Add class name
                .catch((err) => {
                  console.warn(
  ...
  const Card = ({
    ...
    >
      {/* Original styling */}
      <h2 className="text-xl font-bold mb-3 text-purple-700 dark:text-lavenderAccent">
        {title}
      </h2>
      {/* Original styling */}
        ...
        <Card title="Classes" link="/classes" className="md:col-span-4">
          <div className="flex flex-wrap gap-2">
            ...
            {userClasses.length > 0 ? (
              userClasses.slice(0, 8).map(
                (
                  cls // Show limited number on dashboard
                ) => (
                  <span
                    key={cls._id}
                    // Original styling
        ...
        <Card title="Tasks" link="/tasks" className="md:col-span-6">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {dashboardTasks.length > 0 ? (
              ...
              <p className="text-gray-400 dark:text-gray-500 sm:col-span-3 text-center py-4">
